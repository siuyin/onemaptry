...
func main() {
	...
	fmt.Printf("Nearest Bus: %s\n", auth.RetryOnUnauth(searchBus, lat, lng, rad))
}
...
func searchBus(args ...any) ([]byte, error) {
	url := fmt.Sprintf("%s/api/public/nearbysvc/getNearestBusStops?latitude=%s&longitude=%s&radius_in_meters=%s", baseURL, args[0].(string), args[1].(string), args[2].(string))
	resp, err := auth.Get(url)
	if err != nil {
		return []byte{}, fmt.Errorf("auth request: %v", err)
	}
